    ...
    def handle(self, *args, **options):
        ...
        for post in posts:
            try:
                ...
                logger.info(
                    "sendpost.py: Post {}-{} will begin sending. ".format(post.slug, post.id))
                ...
                if (timezone.now() - timezone.make_aware(send_date)) > send_grace_period:
                    try:
                        ...
                        logger.error(
                            "Would have sent more than 20 minutes late")
                        ...
                        logger.critical(
                            "Something is very wrong in sendpost.py ")
                        logger.critical(
                            "Something is very wrong in sendpost.py " + str(traceback.format_exc()))
                        post.log(traceback.format_exc())

                    logger.info(
                        "sendpost.py: Post {}-{} failed to send because it would been late. ".format(post.slug, post.id))
                    # skip this iteration of the loop so this post will not be posted.
                    continue
                ...
                if post.post_facebook:
                    ...
                    if (post.section.facebook_page_id and post.section.facebook_key):
                        fb_url = call_command('sendfacebook', smpost=post, section=post.section,
                                              url=send_url[1], photo_url=photo_url, fb_default_photo=fb_default_photo)
                    ...
                    if (post.section.also_post_to and
                        ...
                        call_command('sendfacebook', smpost=post, section=post.section.also_post_to,
                                     url=send_url[1], photo_url=photo_url, fb_default_photo=fb_default_photo)
                # Post to twitter
                if post.post_twitter:
                    # Section's account
                    if (post.section.twitter_access_key and post.section.twitter_access_secret):
                        tweet_url = call_command(
                            'sendtweet', smpost=post, section=post.section, url=send_url[1], photo_url=photo_url)
                    # Also post to account
                    if (post.section.also_post_to and
                            post.section.also_post_to.twitter_access_key and post.section.also_post_to.twitter_access_secret):
                        call_command(
                            'sendtweet', smpost=post, section=post.section.also_post_to, url=send_url[1], photo_url=photo_url)
            except:
